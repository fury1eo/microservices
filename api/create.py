...
def insertLesson(psql, type):
    return psql.execute(
        f"INSERT INTO {utils.TABLE_LESSONS} (type) \
            VALUES ('{type}');"
    )
...
def fillSchedule(psql, group, lessons):
    ...
    for i in range(WEEKS_TO_FILL):
        lesson = random.choices(lessons, k=3)
...
def fillVisits(psql, schedule):
    for student in filter(lambda student: student["group"] == schedule["group_fk"], STUDENTS):
